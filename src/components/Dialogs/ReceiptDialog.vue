  ...
  <v-dialog v-model="$store.state.receipt.receiptDialog">
    <v-card color="var(--main-bg-color)">
      <form @submit.prevent="handleSubmit()">
        <v-card-title>
          <span class="ml-5"> Ingrese la información del comprobante </span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row class="ma-5">
            ...
            <v-row v-if="dialogObject.documento.tipo === true">
              <v-col cols="12" class="ml-5 text-center">
                <span class="title">
                  Seleccione un comprobante asociado a "{{
                    dialogObject.documento.nombre
                  }}"
                </span>
              </v-col>
              <v-col class="d-flex">
      ...
      <v-dialog
        v-if="activeDetailDialog"
        v-model="activeDetailDialog"
        width="800px"
        @input="activeDetailDialog = false"
      >
        <v-card>
          <v-card-title>
...
export default {
  ...
  computed: {
    totalDetail() {
      return this.detailRelationateReceipt.reduce(
        (acc, el) => acc + el.precioTotal,
        0
      );
    },
  },
    ...
    handleSubmit() {
      if (this.dialogObject.documento.tipo === false) {
        ...
        if (!this.dialogObject.comprobanteAsociado) {
          this.$errorAlert(
            "Si emite un comprobante fiscal debe seleccionar un documento fiscal relacionado"
          );
        } else {
          this.$store.commit("receipt/addReceipt", this.dialogObject);
          this.$emit("receipt", this.$store.state.receipt.receipt);
    ...
    filterObjects() {
      GenericService(this.tenant, "ventas", this.token)
        ...
        .then((data) => {
          ...
          if (
            this.filterParams.ventas.totalPages < this.filterParams.ventas.page
          ) {
